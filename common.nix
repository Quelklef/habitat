...
my-lib = import ./files/my-lib.nix { inherit pkgs lib; };
inherit (my-lib) linked linked-script;

result = { inherit parts folded; };
...
desktop-environments = {
  ...
  imports = [
    # Enable COSMIC DE
    (import ./files/de-cosmic.nix { inherit pkgs lib perloc stateloc user host; })
    # Enable XMonad DE (actually just a WM)
    (import ./files/de-xmonad.nix { inherit pkgs lib perloc stateloc user host; })
  ];

  # Enable a greeter to choose b/w the DEs
  services.displayManager = {
    cosmic-greeter.enable = true;
    defaultSession = "cosmic";
  };
  # Fix: Without this, cosmic-greeter is unable to start xmonad
  environment.systemPackages = [ pkgs.xinit ];

  # # Use auto-login instead of a greeter
  # services.displayManager = {
...
- Run 'borg list' to list available archives

- Then use EITHER

  - 'borg extract --progress ::<archive-name> ./local/path'
    to directly download a chosen archive to local; OR

  - 'borg mount ::<archive-name> ./local/path'
...
- If running this process from a machine not authenticated
  to the Hetzner box, you will need to reset the Hetzner box password
  and go from there.

  Note that Hetzner boxes expose SSH on port 23,
